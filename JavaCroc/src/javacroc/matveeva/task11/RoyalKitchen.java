...
class RoyalKitchen {
    ...
    public boolean hireChef(Chef chef) {
        if (!chefDishesMap.containsKey(chef)) {
            chefDishesMap.put(chef, new HashSet<>());
            return true;
        }
        return false;
    }

    public boolean fireChef(Chef chef) {
        return chefDishesMap.remove(chef) != null;
    }
    ...
    public void updateChefDishes(Chef chef) {
        if (chefDishesMap.containsKey(chef)) {
            ...
            System.out.println("Этот повар здесь не работает");
        }
    }
